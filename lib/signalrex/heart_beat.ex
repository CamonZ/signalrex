defmodule Signalrex.HeartBeat do
  ...
  def init(args) do
    Process.send_after(
      self(),
      :loop,
      time_out_millis(args.keep_alive_timeout)
    )

    {:ok, Map.put(args, :heart_beat, false)}
  end

  def handle_cast(:heart_beat, state),
    do: {:noreply, %{state | heart_beat: true}}

  def handle_cast(_msg, state), do: {:noreply, state}

  def handle_info(
        :loop,
        %{heart_beat: heart_beat, keep_alive_timeout: keep_alive_timeout} =
          state
      ) do
    if heart_beat do
